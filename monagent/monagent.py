    ...
    def handle(self):
        ...
        if method == 'GET':          
            ...
            if CType == 'json' and Alert == 'grafana':
                ...
                info=dicjson['title'] +":"+ dicjson['message'] +":"+ eval(d) for d in dicjson['evalMatches']
                ...
                for uf in UpFiles :           
                    if dicmess['secid'] == secid and dicmess['type'] == uf :
                        ...
                        if dicmess['type'] == 'baseinfo' or dicmess['type'] == 'k8sinfo' :
                            (status,novalue) = commands.getstatusoutput('truth=`grep ' +dicmess['ip']+ ' /root/log/'+uf+'`;if [ -n "${truth}" ]; then sed -i \'/'+dicmess['ip']+'/d\' /root/log/'+uf+'; fi')
                            print "status:",status,":",novalue
